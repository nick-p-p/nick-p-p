...
// int SysConfigHandler::uniqueID()
// {
//     return SYSTEM_DATA_UID;
// }
...
void SysHandler::webPageData(AsyncWebServerRequest *request)
{
    ...
    {
        pageProperties.add("tempunits", (int)MyGlobals::useCelsius());
        pageProperties.add("msgdebug", MyGlobals::messageDebug());
        pageProperties.send(request);
    }
...
void SysHandler::servePageAction(AsyncWebServerRequest *request, String action)
{
    ...
    if (action == "download")
    {
        ...
        {
            pageProperties.sendConfirmation(request, Language::phrase(PHRASE_PHRASE_NOT_FOUND));
        }
    }
...
void SysHandler::savePageData(AsyncWebServerRequest *request)
{
    ...
    MyGlobals::setUseCelsius(request->arg("tempunits") != "0");
    MyGlobals::setMessageDebug(request->arg("msgdebug").toInt());
