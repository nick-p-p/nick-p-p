...
void TZManager::webPageBlock(AsyncWebServerRequest *request)
{
		int zonesToInclude = zoneCount;
#ifdef ARDUINO_ESP32_POE_ISO
	zonesToInclude--;
	...
	StringBuilder stringBuilder(512);
	stringBuilder.append("<select name='tzone' id='tzone'>");
...
void TZManager::load()
{
	...
	if (!tzFile)
	{
		ILogger::log(l_error, "Failed to open timezone file");
		return;
	}
